    ...
    switch (count($urlArr)) {
        ...
        case 3:
            if (in_array($urlArr[1], array('Mobile', 'Web'))) {
                return U('/addon/' . $urlArr[0] . '/' . strtolower($urlArr[1]) . '/' . $urlArr[2] . '@' . C('HTTP_HOST'),
                    $param);
            } else {
...
function mch_pay($params = array())
{
    ...
    $mchpay       = new MchPay_pub($settings['appid'], $settings['mchid'], $settings['paysignkey'],
        $settings['appsecret']);
...
function redpack_pay($params = array())
{
    ...
    $mchpay       = new Redpack_pub($settings['appid'], $settings['mchid'], $settings['paysignkey'],
        $settings['appsecret']);
...
function reply_music($arr)
{
    ...
    $wechatObj = get_wechat_obj();
    return $wechatObj->music($arr['title'], $arr['description'], $arr['musicurl'], $arr['hgmusicurl'],
        $arr['thumbmediaid'])->reply();
}
 ...
 * @author snkso.com
 */
function get_material_thumb($media_id = null)
{
    if (!$media_id)
        return '';
